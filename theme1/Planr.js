...
const onValueChange = (e) => {
    ...
    if(work_item_column === 'wp-status'){
        if(!Object.keys(statusMap).includes(work_item_column_value)){
            alert('invalid status');
            window.location.reload();
        }
        handleStatusValueChange(e.target);
    }
...
const saveNewWorkItem = () => {
    const modal_ref = document.querySelector('#new_item_modal > .modal >.new-item-container');
    const work_item_id = modal_ref.querySelector('.item-id > input').value;
    const work_item_name = modal_ref.querySelector('.item-name > input').value;
    const work_item_point = modal_ref.querySelector('.item-point > input').value;
    
    if(!work_item_id || !work_item_name || !work_item_point)return;
    
    const new_work_item = {
        "wp-id" : work_item_id,
        "wp-name" : work_item_name,
        "wp-owner" : DEFAULT_OWNER,
        "wp-status" : DEFAULT_STATUS,
        "wp-point" : work_item_point
    }
    let work_item = WORK_ITEM.content.cloneNode(true);
    work_item.querySelector('li').id = work_item_id;
    for(let item_key in new_work_item){
        let contentSelector = '.'+item_key+' > span';
        work_item.querySelector(contentSelector).textContent = new_work_item[item_key];
        if(item_key.indexOf('status') > -1){
            work_item.querySelector('.'+item_key).classList.add(statusMap[new_work_item[item_key]]);   
        }
    }
    document.querySelector('ul.wp-container').appendChild(work_item);
    store.sprints[store.currentSprint].push(new_work_item);
    updateLocalStorage();
    document.querySelector('#new_item_modal').close();
...
const deleteWorkItem = (e) => {
    ...
    const work_item_id = e.currentTarget.parentElement.parentElement.id;
    document.getElementById(work_item_id).remove();
    store.sprints[store.currentSprint] = store.sprints[store.currentSprint].filter(work_item => work_item['wp-id'] !== work_item_id);
    updateLocalStorage();
}